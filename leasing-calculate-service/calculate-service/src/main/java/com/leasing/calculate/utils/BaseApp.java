...
import com.leasing.calculate.Enum.Lease_Cal_Method;
import com.leasing.calculate.Enum.Payment_Frequency;
import com.leasing.calculate.Enum.Year_Days;
...
import com.leasing.calculate.vo.BaseAppVO;
...
/**
 * @project:leasing-cloud 核心算法参数封装功具类
 * @date:2019/10/11
...
public class BaseApp {
     ...
     * @return
     */
    public static TreeMap<Integer, BigDecimal> getTermFlowMap(List<CalArithmeticVO> list, BigDecimal puttingAmount) {
        TreeMap<Integer, BigDecimal> dateFlowMap = new TreeMap<Integer, BigDecimal>();
        // 加入投放支出现金流
        dateFlowMap.put(Integer.valueOf(0), puttingAmount.negate());
        for (CalArithmeticVO vo : list) {
            BigDecimal addValue = vo.getLease_cash();
            // 支出为负
            if (vo.getDirection().intValue() == Direction.PAY.getShort().intValue()) {
            ...
            if (dateFlowMap.containsKey(vo.getLease_time())) {
                BigDecimal initValue = dateFlowMap.get(vo.getLease_time());
                BigDecimal resultValue = initValue.add(addValue);
                dateFlowMap.put(vo.getLease_time(), resultValue);
            } else {
                dateFlowMap.put(vo.getLease_time(), addValue);
            }
        }
        return dateFlowMap;
    }
    ...
    public static List<ArithmeticCoreDayParam> getDayParam(ArithmeticCoreParam arithmeticParm) {
        ...
        List<String> calDateList = arithmeticParm.getCalDateList(); //还款期数列表
        int days = 360;
        ...
        Short payment_frequency = arithmeticParm.getPayment_frequency();//支付频率
     ...
     * @return
     */
    public static List<ArithmeticCoreDayParam> getDayPriParam(ArithmeticCoreParam arithmeticParm) {
        List<ArithmeticCoreDayParam> dayParamList = new ArrayList<ArithmeticCoreDayParam>();
        ...
        BigDecimal radio_hyear = arithmeticParm.getRadio_hyear();
        // 每期所占的比例
        BigDecimal term_radio = BigDecimal.ONE;
        for (int i = 1; i <= calDateList.size(); i++) {
            String calDate = calDateList.get(i - 1);
            BigDecimal radio = term_radio;
            int factDay = UFDate.getDaysBetween(UFDate.getDate(puttingDate), UFDate.getDate(calDate));
            ...
            if (payment_frequency.intValue() == Payment_Frequency.HALF_YEAR.getShort().intValue()) {
                if (((i == 1 && UFDate.getDate(calDate).getMonth() > 6) || (i == calDateList.size()))) {
                    dayParamList.add(BaseAppVO.getCoreDayParam(factDay, BigDecimal.valueOf(0.5), dayRate, calDate, beginDate));
                    beginDate = calDate;
                    continue;
                }
                // 正常是两个一起计算
