 ...
 * @description:UFDate日期类型处理工具类 可处理指定间隔后的日期 获取两日期间间隔等等并完成格式转化的工具类
 **/
public final class UFDate implements Serializable {
    ...
    private static Hashtable htMothTo0 = new java.util.Hashtable();
    private int dayTo_0 = 0;
    private int monthTo_0 = 0;
    /**
     * UFDate 构造子注释。
     */
    public UFDate() {
    ...
    public UFDate getMonthAfter(int month){
        Date date = DateUtils.parseDate(toString(), "yyyy-MM-dd");
        date = DateUtils.addMonth(date, month);

        return new UFDate(DateUtils.formatDate(date, "yyyy-MM-dd"));
    }
