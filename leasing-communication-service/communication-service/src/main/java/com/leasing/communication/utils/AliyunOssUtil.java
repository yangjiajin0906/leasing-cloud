...
//        return ossClient;
    ...
    /**
     * 根据文件名 获得文件流
     * @param filePathName    要下载的文件名全路径
     * @param fileName    文件名
     */
    public static OSSObject downloadFile(String filePathName) throws IOException {
        // 创建OSSClient实例。
        OSSClient ossClient = getOSSClient();
        // 下载OSS文件到本地文件。如果指定的本地文件存在会覆盖，不存在则新建。
        OSSObject object = ossClient.getObject(new GetObjectRequest(bucketName, filePathName));
        return object;
    }
     ...
     * 列举 test 文件下所有的文件
     */
    public static List<OSSObject> listFile() {
        // 创建OSSClient实例。
        OSSClient ossClient = getOSSClient();
        ...
        // 设置prefix参数来获取2019-11-22目录下的所有文件。
        listObjectsRequest.setPrefix("temp/2019-11-22/");
        // 列出文件。
